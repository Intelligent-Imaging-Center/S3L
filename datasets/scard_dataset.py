...
from utils.camera_params_reader import parse_camera_params, get_camera_params_for_dataset
import os.path as osp

width_to_baseline = dict()

width_to_baseline[1024] = 0.05
K_of_ENDOSCOPY = None
CAMERA_PARAMS = None

def load_camera_params():
    global CAMERA_PARAMS, K_of_ENDOSCOPY
    if CAMERA_PARAMS is None:
        params_file = osp.join(osp.dirname(osp.dirname(osp.abspath(__file__))), 
                              'classification_result.txt')
        CAMERA_PARAMS = parse_camera_params(params_file)
        K_of_ENDOSCOPY = CAMERA_PARAMS[1]['M1']
        print(f"load {len(CAMERA_PARAMS)} camera")



K_of_SCARED = [[721.54, 0, 609.56, 0], [0, 721.54, 172.85, 0], [0, 0, 1, 0]]

@platform_manager.DATASETS.add_module
class SCAREDColorDepthDataset(data.Dataset):
    DATA_NAME_DICT = {
        'color_l': ('left', 'png'),
        'color_r': ('right', 'png'),
        'depth': ('disp', 'png'),
    ...
    def __init__(
        ...
        load_camera_params()
        self.dataset_dir = Path.get_path_of('scared')
        ...
        if self.jpg_test:
            if dataset_mode == 'test':
                self.dataset_dir = Path.get_path_of('eigen_scared_test_jpg')
            else:
    ...
    def __getitem__(self, f_idx):
        ...
        dataset_id = getattr(self, 'current_dataset_id', None)
        if dataset_id is not None and CAMERA_PARAMS is not None:
            camera_param = get_camera_params_for_dataset(dataset_id, CAMERA_PARAMS)
            if data_side == 'l':
                intrinsic = np.array(camera_param['M1'])
            else:
                intrinsic = np.array(camera_param['M2'])

            baseline = camera_param['baseline']
            width_to_baseline[img_W] = baseline
        elif self.is_fixK:
            intrinsic = np.array(K_of_SCARED)
        else:
            intrinsic = np.array(K_of_SCARED)
        ...
        if self.load_depth:
            ...
            depth_data = self.get_input_depth_from_disp(depth_path, data_side)
            inputs['depth'] = depth_data
            inputs['disp'] = torch.from_numpy(depth_data.copy()).unsqueeze(0)
    ...
    def _get_formative_file_base(self, info):
        ...
        import re
        dataset_match = re.search(r'dataset_(\d+)', data_dir)
        if dataset_match:
            self.current_dataset_id = int(dataset_match.group(1))
        else:
        ...
        base_path_list = []
        base_path = os.path.join(self.dataset_dir, data_dir, '{}', 
                                'frame_data{}'.format(data_idx) + '.{}')
        base_path_list.append(base_path)

        for frame_id in self.output_frame:
            if frame_id != 'o':
                frame_idx = int(data_idx) + frame_id
                base_path = os.path.join(self.dataset_dir, data_dir, '{}',
                                        'frame_data{}'.format(frame_idx) + '.{}')
                base_path_list.append(base_path)
            else:
                base_path = os.path.join(self.dataset_dir, data_dir, '{}',
                                        'frame_data{}'.format(data_idx) + '.{}')
    ...
    def get_input_depth_from_disp(self, disp_path, data_side):
        try:
            disp_img = get_input_img(disp_path, False)
            disp_data = np.array(disp_img, dtype=np.float32)
            ...
            if self.frame_counter % 100 == 0:
                print(f"disp data: min={disp_data.min()}, max={disp_data.max()}, mean={disp_data.mean()}")

            return disp_data
        except Exception as e:
            print(f"error: {e}")
            default_disp = np.ones((1024, 1280), dtype=np.float32) * 50
            return default_disp
